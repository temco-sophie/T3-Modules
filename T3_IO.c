...
//	signed int aa;
	...
	if(flash_read_int(FLASH_DEAD_MASTER, &guiDeadmasterTime, FLASH_MEMORY))
	{
		gucDeadmaster = guiDeadmasterTime;
		guiDeadmasterTime = (unsigned int)(gucDeadmaster << 8);
	}
	else
	{
		gucDeadmaster = 0;
		guiDeadmasterTime = 0;
		flash_write_int(FLASH_DEAD_MASTER, &guiDeadmasterTime, FLASH_MEMORY);
	}
 

	while(1)
	{
		if(gucDeadmaster && (guiDeadmasterTime == 0) && (serial_no_activity == 1))
		{
		//initSerial();		
		}
	...
	if (switch_state_change)
	{
		...
		if ( switch_set == LO)
		{
			...
			for (i = 0; i < 8; i++)
			{
				...
				modbus.registers[EEP_SWITCH_STATE1] = modbus.registers[EEP_SWITCH_STATE1] | switch_state[i];
			}
						//debug 
...
*/
		...
		for (i = 0; i < 8; i++)
		{
			if (switch_state[i] == 2 && modbus.registers[i] > threshold)	// auto mode
				LED_State[0] = LED_State[0] >> 1;
			else if (switch_state[i] == 1)					// hand mode
				LED_State[0] = LED_State[0] >> 1;
			else												// off mode
